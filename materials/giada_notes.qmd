...
df$condition= factor(df$SetSize)
...
# metodo è l'analogo dei "test post-hoc" per le curve
...
#  Interpretazione  Se l'intervallo di confidenza calcolato non include lo zero per certi valori di TDsim, allora in quei punti le curve dei due gruppi sono statisticamente diverse  
...
```{r}
# random intercept Equivalente LMM (1 | Participant)
formula <- RT ~ s(TDsim) + s(Participant, bs = "re")  # base spline "re" random effects. 
...
`s(A, B, bs="re") vs. s(A, by=B, bs="re")`

Nel caso di bs="re" (effetti casuali parametrici), s(TDsim, Participant, bs="re") e s(TDsim, by=Participant, bs="re") sono matematicamente equivalenti e producono entrambi pendenze casuali non correlate, sebbene la sintassi by sia spesso preferita per chiarezza quando si definiscono random slopes.

```{r}

# RT ~ ... + x + s(Participant, x, bs = "re") ← Modellare pendenze diverse per x tra soggetti. Random slope (se il fixed term è lineare) Equivalente LMM (x | Participant)
formula <- RT ~ s(TDsim) + s(TDsim,Participant, bs = "re")  
mod <- gam(formula, data = df, method = "REML"); summary(mod); plot.gam(mod, pages = 1,  all.terms = TRUE, residuals = TRUE, shade = TRUE)
```
